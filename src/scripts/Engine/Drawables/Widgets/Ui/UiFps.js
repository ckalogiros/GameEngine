...
export class UiFpsGeometry{
    ...
    constructor(text, pos, scale, fontSize){
        ...
        const geom1 = new TextGeometry2D(pos, fontSize, scale, text);
        ...
        const geom2 = new TextGeometry2D(pos, fontSize, scale, MAX_FPS_NUMBERS);
    ...
    Update(params){
        ...
        const len = geom.numChars > textFpsAvg.length ? geom.numChars : textFpsAvg.length
        for (let i = 0; i < len; i++) {
            /** Update fps average */
            let uvs = [0,0,0,0];
