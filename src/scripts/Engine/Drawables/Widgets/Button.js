 ...
 */
export function ButtonRunScaleAnimation(idx, scaleFactor, maxScale) {
 ...
 */
function ButtonTextScaleAnimation(idx, scaleFactor, maxScale) {

    let posx = 0;
    let accumTextWidth = 0;
    const textLen = buttons.btn[idx].text.letters.length;

    // const extraWidth = buttons.btn[idx].style.pad * 2 * 1.4;
    const extraWidth = buttons.btn[idx].style.pad * 2;
    // scaleFactor*=.98;
    for (let i = 0; i < textLen; i++) {
         ...
         * but to be added to the next character's (next iteration) starting position  .
        */
        ...
        accumTextWidth += buttons.btn[idx].text.letters[i].dim[0];
        GlSetDim(buttons.btn[idx].text.letters[i].gfxInfo, buttons.btn[idx].text.letters[i].dim);

        // console.log('Button scale', buttons.btn[idx].text.dim[1])
        const sdfOuter = CalculateSdfOuterFromDim(buttons.btn[idx].text.letters[i].dim[1], maxScale*buttons.btn[idx].text.letters[i].dim[1]);
...
export function ButtonSetDefaultMeshProps(idx) {
    ...
    buttonsArea[idx].scale[0] = buttons.btn[idx].area.defScale[0]; // Reset button's area scale x
    buttonsArea[idx].scale[1] = buttons.btn[idx].area.defScale[1]; // Reset button's area scale y
    ...
    for (let i = 0; i < textLen; i++) {
        ...
        const sdfOuter = CalculateSdfOuterFromDim(buttons.btn[idx].text.letters[i].dim[1], null);
        GlSetAttrSdfParamsOuter(buttons.btn[idx].text.letters[i].gfxInfo, sdfOuter);
    }
}
