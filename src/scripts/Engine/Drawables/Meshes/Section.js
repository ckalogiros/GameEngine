...
export class Section extends Rect {
   ...
   Calc(options = SECTION.INHERIT) {
      ...
      if(options & SECTION.TOP_DOWN){
         ...
         // console.log('max-size:', section.max_size[1], 'section-dim:', section.geom.dim[1], ' old-size:', old_size_y, ' btn-h:', btn_h)
         // section.SetPosY(y);
         section.SetPosXY([x, y]);
      }
      // else {
      //    // const btn_h = section.children.buffer[0].geom.dim[1];
      //    // // const h = section.geom.pos[1]-(section.geom.dim[1]*2+btn_h-8.4);
      //    // // const h = section.geom.pos[1]-(section.geom.dim[1]*2+btn_h);
      //    // const h = section.geom.pos[1]-(section.geom.dim[1]+btn_h);
      //    // // const h = section.geom.pos[1]-(section.geom.dim[1]+btn_h);
      //    // const dif = section.geom.pos[1] - h
      //    // console.log('max-size:', section.max_size[1], 'section-dim:', section.geom.dim[1], ' old-size:', old_size_y, ' btn-h:', btn_h)
      //    // // section.SetPosY(h);
      //    // section.SetPosY(section.geom.pos[1] - dif);
         
      //    // const d = section.geom.dim[1] - old_size_y - btn_h;
      //    const d = section.geom.dim[1];
      //    // const d = section.geom.dim[1] - old_size_y;
      //    console.log(section.name, section.children.buffer[0].geom.defPos[1], 'd:', d)
      //    // section.SetPosY(section.children.buffer[0].geom.defPos[1]-d);
      //    section.SetPosY(section.children.buffer[0].geom.defPos[1]);
      // }
      section.SetMargin()
...
//          if ((opt & SECTION.HORIZONTAL) && (opt & SECTION.VERTICAL)) {
//             // accum_size[0] += mesh.geom.dim[0] * mesh.mat.num_faces;
//             // accum_size[1] += mesh.geom.dim[1]; console.log(accum_size[0])
//             section.max_size[0] += (mesh.geom.dim[0]/2 * mesh.mat.num_faces);
//             section.max_size[1] += mesh.geom.dim[1]/2;
//             // if (section.max_size[0] < accum_size[0]) 
//             //    section.max_size[0] = mesh.geom.dim[0] * mesh.mat.num_faces;  // Keep the max width of all meshes
//             // if (section.max_size[1] < accum_size[1]) 
//             //    section.max_size[1] = mesh.geom.dim[1];  // Keep the max height of all meshes
//          }
//          else if (opt & SECTION.VERTICAL) {
//             accum_size[1] += mesh.geom.dim[1];
//             accum_size[0] = mesh.geom.dim[0] * mesh.mat.num_faces;
//             section.max_size[1] += mesh.geom.dim[1];
//             if (section.max_size[0] < accum_size[0]) 
