export const FS_V2DGFX_ORI = `#version 300 es
...
void init (vec2 fragCoord, vec2 mouse, vec2 resolution) {
	...
	ScreenH = min(resolution.x,resolution.y)*2.5;
	AA = ScreenH*0.5;
	AAINV = 1.0 / AA;
...
void rounded_rectangle(vec2 o, vec2 s, float r) {
    ...
    vec4 dmin = min(d,-0.04);
    vec4 dmax = max(d,-0.0);
    vec2 df = max(dmin.xz, dmin.yw) + length2(dmax);
    //vec2 df = max(dmin.xz, dmin.yw) + length2(dmin);
    //vec2 df = dmin.xz + .1;
    add_field(df - r);
}
...
void paint(float t) {
    ...
    //rounded_rectangle(-0.1,-0.2,0.9,0.9, mix(0.0,1.,sin(t)*0.5+0.5));
    rounded_rectangle(-0.1,-0.2,0.9,0.9, mix(0.0,1.,sin(t)));

    if (in_fill()) 
    {
        // animate the texture a little
        save(ctx);
        translate(mod(t*0.2,1.0), 0.0);
